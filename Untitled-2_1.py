...
def net_builder_fn(num_classes):
    ...
    model = wrn_builder.build(num_classes=num_classes)
    return model
...
def train_flexmatch_one_round(
    ...
    flexmatch_obj = FlexMatch(
        net_builder=model_builder,  # e.g. lambda num_classes: wrn_builder.build(num_classes)
    ...
    optimizer = get_optimizer(flexmatch_obj.model.parameters(), args.optim, args.lr, args.momentum, args.weight_decay)
