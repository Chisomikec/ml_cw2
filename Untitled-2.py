...
# %%
def tpc_rp_selection(
    unlabeled_indices,dataset,B=10,device='cpu',embedding_mode='selfsupervised',simclr_model=None,
    classifier=None
):
    """
    A universal function for TPC(RP) that can use either
    (a) self-supervised SimCLR embeddings, or
    (b) the classifier's penultimate-layer embeddings.
    """
    # 1) Extract embeddings
    if embedding_mode == 'selfsupervised':
        assert simclr_model is not None, "Need a simclr_model for self-supervised"
    ...
    elif embedding_mode == 'fullysupervised':
        assert classifier is not None, "Need a classifier for fully supervised"
        unlabeled_subset = Subset(dataset, unlabeled_indices)
        embeddings = extract_embeddings_from_classifier(classifier, unlabeled_subset, device=device)
    else:
...
def active_learning_loop(
    selection_fn,         # function to pick new points (TPC(RP) or random)
    simclr_model,         # only needed if selection_fn uses embeddings
    trainset,
    test_loader,
    initial_B=10,
    cycle_B=10,
    ...
    """
    Integrates the initial selection + iterative queries.
    'selection_fn' can be 'initial_pool_selection' for TPC(RP),
    or 'random_selection' for random queries, etc.
    """
    # Start with no labeled data
    unlabelled_indices = list(range(len(trainset)))
    labeled_indices   = []

    # A. Initial pool selection
    init_selection = selection_fn(simclr_model, unlabelled_indices, trainset, B=initial_B, device=device)
    for idx in init_selection:
        unlabelled_indices.remove(idx)
        labeled_indices.append(idx)
    ...
    accuracies = []
    for cycle in range(cycles):
        # 1) Train a supervised model from scratch using the labeled_indices
        classifier = SimpleClassifier(num_classes=10)  
        train_supervised(classifier, trainset, labeled_indices, device=device)
        ...
        # 3) Query new points using the same selection function
        new_selection = selection_fn(simclr_model, unlabelled_indices, trainset, B=cycle_B, device=device)
        
        # 4) Add them to labeled set
        for idx in new_selection:
            unlabelled_indices.remove(idx)
            labeled_indices.append(idx)

    # Plot the accuracy curve
    plt.plot(range(len(accuracies)), accuracies, marker='o', label=selection_fn.__name__)
    plt.xlabel("Active Learning Cycle")
    plt.ylabel("Test Accuracy (%)")
    plt.title(f"{selection_fn.__name__} Accuracy over AL Cycles")
    plt.legend()
    plt.show()
